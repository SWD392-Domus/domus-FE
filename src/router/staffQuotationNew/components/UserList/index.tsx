...
import { getAllUser } from "../../usecase/getAllUser";
...
const UserList: React.FC<Props> = ({ open, setOpen }) => {
    const placeholder =
    ...
    const fetchData = async () => {
        ...
        const userList = await getAllUser(token as string);
        setUsers(userList);
    };
    const handleSelect = (item: User) => {
        dispatch(actions.setUser(item as any));
        setOpen(false);
    };
    useEffect(() => {
        fetchData();
    ...
    return (
        <CommandDialog open={open} onOpenChange={setOpen}>
            <Command className="rounded-lg border shadow-md">
                <CommandInput placeholder="Customer Search...." />
                <CommandList>
