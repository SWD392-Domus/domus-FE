      ...
      if (quotationId && isMounted) {
        try {
          ...
          if (response) {
            ...
            setUpdated(true);
            const fetchedProducts: ProductDetailProps[] = response.products;
            const fetchedServices: ServiceProps[] = response.services;
            ...
            fetchedServices.forEach((service, index) => {
              ...
              serviceInitialValues[index.toString()] = serviceCellValues;
            });
    ...
    function validateCellValues(cellValues: CellValues): boolean {
      for (let key in cellValues) {
        ...
        for (let nestedKey in nestedObject) {
          if (!nestedObject[nestedKey] || nestedObject[nestedKey] === "") {
            ...
            <div className="staff-assigned-info my-7">
              ...
              {!isEditStaff ? (
                ...
                >
                  ...
                  <h2 className="my-auto font-medium">{staff.userName}</h2>
                </div>
              ) : (
                <ChooseStaff setEditStaff={setEditStaff}/>
              )}
            </div>
          </div>
            ...
            <div className="border-solid border-b-2 border-zinc-400">
                  ...
                  <div className="flex flex-row flex-wrap w-full">
                    <div className="left-side-3 basis-1/3 ">
                      ...
                      <div className="text-md">
                        <div className="flex flex-row">
                          <div className="font-semibold mr-1">Exprie Date</div>
                          <p>{new Date(expireAt).toDateString()}</p>
                        </div>
                      </div>
                    </div>
                    <div className="right-side-3 basis-2/3 font-sans text-md">
                      ...
                      <div className="user-info-body text-md">
                        <div className="phone-info flex flex-row mb-1">
                          ...
                          <span className="">{customer.phoneNumber}</span>
                        </div>
                      </div>
                    </div>
                  </div>
              ...
              <div className="detail-table">
                <div className="mt-7 text-xl font-semibold flex items-center">
                  ...
                  <Button
                    variant={"default"}
                    onClick={() => setEditTable(!isEditTable)}
                    className="mr-2"
                  >
                ...
                <div className="mx-auto py-5">
                  {isEditTable ? (
                    ...
                    <DataTable columns={column} data={products as any} />
                  )}
                </div>
              </div>
              <div className="detail-table">
                <div className="mt-7 text-xl font-semibold  flex items-center">
                  ...
                  <Button
                    variant={"default"}
                    onClick={() => setEditService(!isEditService)}
                    className="mr-2"
                  >
                    {isEditService ? "Cancel" : "Edit"}
                  </Button>
                  {isEditService && (
                    <Button variant={"default"} onClick={handleSaveService}>
                      Save
                    </Button>
                  )}
                </div>
                <div className="mx-auto py-5">
                  {isEditService ? (
                    <>
                      <EditTableService
                        columns={editServiceColumns as any}
                        data={services as any}
                        serviceCellValues={serviceCellValues}
                        setServiceCellValues={setserviceCellValues}
                      />
                      <Button
                        variant={"ghost"}
                        className="mt-2 text-blue-00"
                    ...
                    <DataTable columns={serviceColums} data={services as any} />
                  )}
