...
export const quotationStaffApi = {
  ...
  createQuotationCustomer: (data: any, token: string) => {
    return post(`/customer/quotations`, data, {}, { Authorization: token });
  },
  createQuotationStaff: (data: any, token: string) => {
    return post(
      `/staff/quotations?customerId=${data?.customerId}`,
      data,
      {},
      { Authorization: token }
    );
